  ...
  <div v-if="steps.length" class="closure-trace">
    <div class="closure-trace-head">
      ...
      <div v-if="closure" class="closure-trace-summary">
        ...
        <span v-if="closure.amount"
          >· ¥{{ closure.amount.toLocaleString('zh-CN') }}</span
        >
      </div>
    </div>
      ...
      >
        <div class="closure-timeline-rail">
          <div class="closure-timeline-dot" :class="stepIconClass(step.key)">
            <el-icon v-if="step.status === 'error'"
              ><CircleCloseFilled
            /></el-icon>
            <el-icon v-else-if="step.key === 'approval_pass'"
              ><CircleCheckFilled
            /></el-icon>
            <el-icon v-else-if="isSealStep(step.key)"><Stamp /></el-icon>
            <el-icon v-else-if="step.key.startsWith('contract')"
              ><Document
            /></el-icon>
...
const isComplete = computed(() => {
  ...
  return props.steps.some(
    (step) => step.key === 'seal_b' && step.status === 'done'
  )
})
</script>
