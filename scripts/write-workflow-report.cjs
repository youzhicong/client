...
const target = path.join(__dirname, '../src/views/ai-workflow/report.vue')

const content = `<template>
  <div class="workflow-report-page ai-agent-shell">
    <AgentStudioHeader
      title="工作流可视化报告"
      description="Multi-Agent 全流程产出 · 可分享、可导出"
      active="workflow"
      compact
    >
      <template #actions>
        <button v-if="report" class="ai-agent-btn-ghost" type="button" @click="exportMarkdown">
          导出 Markdown
        </button>
        <button v-if="report" class="ai-agent-btn-primary" type="button" @click="continueInChat">
          Agent 聊天继续
        </button>
        <button class="ai-agent-btn-ghost" type="button" @click="goWorkflow">
          返回工作流
        </button>
      </template>
    </AgentStudioHeader>
    ...
    <div v-if="!report && !loading" class="report-empty">
      ...
      <button type="button" class="ai-agent-btn-primary" @click="goWorkflow">去运行工作流</button>
    </div>
    ...
    <div v-else-if="report" class="report-body">
      <header class="report-hero">
        <div class="report-hero-main">
          ...
          <p class="report-subtitle">{{ report.summary || 'AI 已完成市场研究、创意生成与评估优化。' }}</p>
        </div>
        ...
        <time class="report-time">生成于 {{ formatTime(report.timestamp) }}</time>
      </header>
      ...
      <section class="report-section">
        ...
        <div class="report-pipeline">
          ...
          >
            ...
            <div v-if="index < pipelineAgents.length - 1" class="node-connector" />
          </div>
        </div>
      </section>
          ...
          <div class="matrix-grid">
            ...
            >
              <div class="matrix-card-head">
                ...
                <span v-if="isTopPick(product.name)" class="top-badge">TOP</span>
              </div>
              <p>{{ product.description }}</p>
              <footer>
                <span class="potential" :class="potentialClass(product.marketPotential)">
...
import type { BusinessClosureResult } from '@/services/ai-automation/business-flow'
import {
...
const closureSteps = computed(() => report.value?.businessSnapshot?.closureSteps || [])
...
.report-hero {
  ...
  background: linear-gradient(135deg, #eff6ff 0%, var(--app-surface) 55%, #ecfdf5 100%);
}
